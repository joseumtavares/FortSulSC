...
export function CategoryDeleteButton({ categoryId }: { categoryId: string }) {
  ...
  async function handleDelete() {
    ...
    setError('')
    try {
      const response = await fetch(`/api/admin/categories/${categoryId}`, { method: 'DELETE', credentials: 'same-origin' })
      const data = await response.json() as { error?: string }
      if (!response.ok) { setError(data.error ?? 'Não foi possível excluir a categoria. Tente novamente em instantes.'); return }
