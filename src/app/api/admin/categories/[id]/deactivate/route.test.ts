...
function request() {
  return new NextRequest('http://localhost:3000/api/admin/categories/cat-1/deactivate', { method: 'POST' })
}

import { POST } from './route'

describe('deactivate category', () => {
  it('returns 404 for a missing category', async () => {
    mocks.find.mockResolvedValue(null)
    expect((await POST(request(), context)).status).toBe(404)
  })

  it('deactivates, audits and revalidates the home', async () => {
    const response = await POST(request(), context)
    expect(response.status).toBe(200)
    expect(mocks.update).toHaveBeenCalledWith(id, false)
    expect(mocks.audit).toHaveBeenCalledWith(expect.objectContaining({ action: 'DEACTIVATE', entityType: 'CATEGORY' }))
