...
describe('banner upload', () => {
  it('denies requests without permission', async () => {
    ...
    expect((await POST(request('POST', form))).status).toBe(401)
  ...
  it('uploads, persists and audits', async () => {
    const form = uploadForm()
    expect((await POST(request('POST', form))).status).toBe(201)
    expect(mocks.create).toHaveBeenCalled()
    expect(mocks.audit).toHaveBeenCalledWith(expect.objectContaining({ action: 'CREATE', entityType: 'BANNER', entityId: id }))
...
it('lists active and inactive banners', async () => {
  mocks.list.mockResolvedValue([{ id, active: true }, { id: 'other', active: false }])
