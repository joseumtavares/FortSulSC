...
describe('deactivate', () => {
  ...
  it('changes only active and audits the correct action', async () => {
    ...
    expect(mocks.update).toHaveBeenCalledWith(id, { active: false })
    expect(mocks.audit).toHaveBeenCalledWith({ adminUserId: 'admin-1', action: 'DEACTIVATE', entityType: 'BANNER', entityId: id, result: 'SUCCESS' })
  })
})
