...
function request() {
  return new NextRequest('http://localhost:3000/api/admin/partners/p1/deactivate', { method: 'POST' })
}

import { POST } from './route'

describe('POST /api/admin/partners/[id]/deactivate', () => {
  it('returns 404 for a missing partner', async () => {
    mocks.find.mockResolvedValue(null)
    expect((await POST(request(), context)).status).toBe(404)
  })

  it('deactivates, audits', async () => {
    const response = await POST(request(), context)
    expect(response.status).toBe(200)
    expect(mocks.updateActive).toHaveBeenCalledWith(id, false)
    expect(mocks.audit).toHaveBeenCalledWith(expect.objectContaining({ action: 'DEACTIVATE', entityType: 'PARTNER' }))
  })
})
