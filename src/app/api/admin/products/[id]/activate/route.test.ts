...
function request() {
  return new NextRequest('http://localhost:3000/api/admin/products/p1/activate', { method: 'POST' })
}
...
describe('activate product', () => {
  ...
  it('activates, audits and revalidates', async () => {
    const response = await POST(request(), context)
    expect(response.status).toBe(200)
    expect(mocks.update).toHaveBeenCalledWith(id, true)
    expect(mocks.audit).toHaveBeenCalledWith(expect.objectContaining({ action: 'ACTIVATE', entityType: 'PRODUCT' }))
