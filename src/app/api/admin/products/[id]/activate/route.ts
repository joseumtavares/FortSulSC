...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await updateProductActive(id, true)
    await recordAuditEvent({ adminUserId: guard.session.user.id, action: 'ACTIVATE', entityType: 'PRODUCT', entityId: id, result: 'SUCCESS' })
    revalidatePath('/')
    return NextResponse.json({ id, active: true })
  } catch {
    logger.error('content.product_activate_failed')
