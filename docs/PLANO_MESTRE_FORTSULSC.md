...
| 🟢 | Fase 4 — Painel administrativo | Encerrada formalmente em 12/09/2026, por decisão de Jose. CRUD completo (produtos, categorias, artigos, banners, configurações institucionais, parceiros/representantes/revendas, áreas comerciais/regiões) implementado, testado e auditado quanto a segurança. | — |
| 🟡 | Fase 5 — Mapa e dados públicos | Iniciada em 12/09/2026. Seleção explícita de campos públicos de `Partner` aprovada por Jose; `GET /api/public/partners` e `RepresentativeMapPanel` (painel deslizante com mapa Leaflet, geolocalização e busca por cidade) implementados na mesma sessão. | Rate limiting hoje é básico (por IP, 30 req/min) — anti-scraping mais robusto e cache seguem pendentes para fechamento formal da fase. |
...
| Revendas/representantes terão página pública própria, acessada pelo botão "Encontrar representante" dentro da seção Presença: mapa Leaflet à esquerda (marcador aparece ao selecionar um representante) e, à direita, os dados do representante selecionado (nome, telefone, redes sociais, link e logotipo) | Aprovada como especificação de produto (Jose, 30/08/2026); **superada em 12/09/2026** — em vez de página dedicada, Jose aprovou um painel deslizante sobre a própria Home (mesmo mapa/dados, layout de painel em vez de rota própria), implementado nesta sessão como `RepresentativeMapPanel` (ver entrada de 12/09/2026 e `COMPONENTS.md` 3.16) |
...
arquiteturalmente a edição via painel (30/08/2026, Parte II). Encerrada
formalmente por Jose em 12/09/2026, com todo o CRUD do objetivo acima
implementado, testado e revisado (ver entradas de 11 e 12/09/2026).

## Fase 5 — Mapa e dados públicos 🟡

Objetivo: implementar o mapa interativo de representantes com Leaflet + OpenStreetMap. A especificação original (Parte II, 30/08/2026: página dedicada, mapa à esquerda/dados à direita) foi substituída em 12/09/2026 por decisão de Jose — painel deslizante sobre a própria Home, com geolocalização do visitante e busca por cidade, implementado nesta sessão (`RepresentativeMapPanel`, ver `COMPONENTS.md` 3.16 e entrada de 12/09/2026).
...
| 11/09/2026 | Antes de decidir o encerramento formal da Fase 4, Jose pediu auditoria de segurança contra um checklist de 14 itens (RLS, chaves de API, rate limit, validação server-side, queries parametrizadas, hash de senha, cookies HttpOnly, expiração de token, 2FA, CORS, controle de permissão, validação de upload, atualização de dependências, proteção de rotas). Agente Explore auditou os 14 itens com citação de arquivo/linha: 11 já estavam corretos (RLS completo, sem chaves expostas, rate limit no login/MFA por design, validação server-side em toda rota, Prisma sempre parametriza consultas, senha com bcrypt custo 12, cookies HttpOnly/Secure/SameSite corretos, sessão de 2h + `tokenVersion` + logout por inatividade, 2FA real com código por e-mail em canal separado e hash com pepper, CSRF/CORS via checagem de mesma origem em toda rota, todas as 30 rotas administrativas exigem sessão). 3 achados exigiram decisão do Jose: (1) RBAC decorativo — `ADMIN` e `EDITOR` faziam exatamente a mesma coisa em toda rota; (2) validação de upload confiava só no `Content-Type` declarado pelo navegador, sem checar os bytes reais do arquivo; (3) 1 vulnerabilidade HIGH via `npm audit` (`deepmerge-ts` transitivo do `@prisma/config`) sem correção disponível — verificado que até a versão mais nova estável do Prisma (7.10.0, éramos 6.19.3) carrega a mesma dependência vulnerável, então não há update possível hoje; risco existe só em tempo de build/CLI local, nunca em produção. Jose decidiu corrigir os itens (1) e (2) e documentar o (3) como pendência monitorada, sem forçar downgrade/upgrade major do Prisma. Implementado: exclusão física de produto (`DELETE /api/admin/products/[id]`) e edição de configurações institucionais (`PUT /api/admin/institutional-settings`) restritas à role `ADMIN` (`requireAdminRequest(request, ['ADMIN'])`, mecanismo que já existia mas nunca era usado); nova verificação de assinatura de arquivo (`src/lib/storage/image-signature.ts`, magic bytes de JPEG/PNG/WEBP) aplicada nos 4 pontos de upload de imagem do painel (produtos, capa de artigo, galeria de artigo, banner), rejeitando arquivo cujo conteúdo real não bate com o `Content-Type` declarado. `lint`, `typecheck` e `npm run test:unit` (407 testes) e `npm run test:db` (53 testes) aprovados. Pendência aberta e ainda sem decisão: escopo da Fase 4 quanto a CRUD de representantes/revendas (`Partner`) e regiões geográficas (`regions`/`states`/`municipalities`/`commercial_areas`) — hoje sem nenhuma rota/tela administrativa, apesar de estarem no objetivo original da fase (linha 332). |
| 11/09/2026 | Jose decidiu resolver a pendência acima agora, como última fatia antes do encerramento formal da Fase 4: construir o CRUD de representantes/revendas e regiões geográficas. Proposta técnica registrada em `docs/Proposta_Fase4_Fatia_Representantes_Regioes.md`, com achado crítico de arquitetura: `Region`/`State`/`Municipality` são a divisão oficial do IBGE (5 regiões, 27 estados, ~5.570 municípios) — impraticável como CRUD manual, então tratado como dado de referência semeado uma única vez; o que é genuinamente editável pelo admin é `CommercialArea` (agrupamento de negócio) e `Partner`. 5 decisões levantadas na proposta (§7) e respondidas por Jose no mesmo dia, registradas em §8: semear só a Região Sul (PR/SC/RS) por enquanto, não o Brasil inteiro; exclusão física de parceiro com confirmação explícita, restrita a `ADMIN` (mesmo padrão do produto); dados privados (LGPD) no mesmo formulário do parceiro, não em etapa separada; logo do parceiro por upload de arquivo (não link de texto); coordenadas sempre arredondadas automaticamente no servidor para ~1km de precisão. Implementado na mesma sessão, na `main`: migration `20260912000000_fase4_partners_regioes_admin` (novo valor `COMMERCIAL_AREA` em `AuditEntityType`; `Partner.logoKey`, mesmo padrão de `Banner.imageKey`); geografia real da Região Sul (1.191 municípios, dados oficiais da API pública do IBGE) semeada via `prisma/seed.ts`/`prisma/data/geografia-sul.json`; repositórios e rotas administrativas completos para `CommercialArea` (CRUD + vínculo de municípios) e `Partner` (CRUD + ativar/desativar + vínculo de áreas comerciais + dados privados + upload de logo com verificação de assinatura de arquivo); telas `/admin/commercial-areas` e `/admin/partners/representantes|revendas` (rotas por caminho, não por query string, para o destaque de navegação funcionar); menu do painel atualizado, removendo um badge fictício que existia desde a criação do componente. Nenhuma rota pública criada — a exibição pública (mapa de representantes) continua na Fase 5. `lint`, `typecheck`, `npm run test:unit` (499 testes) e `npm run test:db` (57 testes, incluindo 4 novos validando o seed de geografia real e o fluxo completo parceiro+área comercial contra Postgres real) aprovados. `scope-gate-reviewer` (achado de processo: a proposta ainda não tinha as respostas de Jose registradas no documento antes da implementação — corrigido registrando-as em `docs/Proposta_Fase4_Fatia_Representantes_Regioes.md` §8 e nesta entrada, já que as decisões em si já haviam sido dadas por Jose durante a sessão) e `ui-reviewer` executados antes do commit. `ui-reviewer` encontrou um bug real: o filtro de busca do checklist de municípios removia itens do DOM (`.filter()`), então marcar um município, filtrar e limpar a busca perdia a marcação silenciosamente (o checkbox remontava com o valor original do servidor); corrigido trocando a remoção por `hidden` (item nunca desmonta). Também corrigido: `PartnerTypeTabs` usava `role="tablist"`/`role="tab"` para links entre páginas diferentes (uso incorreto de ARIA — trocado por `aria-current="page"`, mesmo padrão de `AdminSidebar`); alt da logo do parceiro era um texto genérico fixo (trocado por `Logo de ${partnerName}`); seção de dados privados (LGPD) ganhou destaque visual mais forte (fundo âmbar, antes só borda tracejada); toque do checkbox de município ajustado de `min-h-9` para o padrão `min-h-11` do projeto; `CountedField`/`CharCounter` documentados em `docs/COMPONENTS.md` (lacuna já existia desde a fatia de limite de caracteres). |
| 11/09/2026 | Migration `20260912000000_fase4_partners_regioes_admin` aplicada em produção via Supabase MCP. A geografia (região Sul + PR/SC/RS + 1.191 municípios) também foi semeada diretamente em produção via `execute_sql` (dado, não schema — mesmo motivo de `prisma/seed.ts` não ser uma migration), já que o build da Vercel não roda seed nenhum; confirmado `regions=1`, `states=3`, `municipalities=1191` em produção, igual ao local. Achado colateral durante a checagem: `list_migrations` do Supabase não lista a migration `20260911040000_fase4_fatia5_produtos_categorias` (Produtos/Categorias/Depoimentos) na tabela de controle do Prisma, mesmo o schema já existindo e funcionando em produção (confirmado nesta sessão) — indício de que essa migration foi aplicada por fora do fluxo do Prisma (SQL Editor manual) sem o registro correspondente em `_prisma_migrations`. Não bloqueia nada hoje, mas é a mesma causa raiz da pendência já registrada de automatizar `prisma migrate deploy` no pipeline da Vercel: enquanto isso não existir, cada fatia nova precisa desse passo manual, e a tabela de controle pode ficar dessincronizada da forma como aconteceu aqui. |
| 12/09/2026 | Jose testou manualmente o painel de parceiros recém-entregue e viu, no seletor de áreas comerciais, 5 áreas com nomes sem sentido para o negócio (`A1`, `A2`, `Cascade`, `Restrict`, `Restrict Mun`), pedindo descrições mais claras. Investigação confirmou que não eram dado real: eram sobras exatas do incidente já registrado acima (dois `npm run test:db` rodando sem querer contra produção) — a suíte de `commercial-area` cria áreas com esses nomes de teste, e uma delas (`test-area-restrict-mun`) teve seu `afterAll` de limpeza interrompido por timeout, deixando as 5 linhas e um vínculo para um parceiro real (José Ubirajara Moreira Tavares, representante) que o próprio Jose havia criado ao testar o painel. Removidos via Supabase MCP apenas o vínculo (`partner_commercial_areas`) e as 5 áreas fantasma (`commercial_area_municipalities` depois `commercial_areas`, por ID exato) — o parceiro real e qualquer outro dado ficaram intactos; confirmado zero `test-%` remanescente em `commercial_areas`, `regions`, `states`, `municipalities` e `partners`. |
| 12/09/2026 | Jose testou o painel de parceiros e foi registrando pontos de melhoria (instrução explícita: registrar tudo, não implementar até ele confirmar), depois autorizou implementar todos de uma vez. Implementado nesta sessão: **(1) link de localização** — novo campo no formulário do parceiro para colar um link compartilhado do Google Maps ou Apple Maps; `src/lib/content/location-link.ts` extrai as coordenadas (regex direto em `@lat,lng`/`?q=`/`?ll=`, ou segue o redirect uma vez para links curtos como `maps.app.goo.gl`, sempre validando o host antes e depois do redirect contra uma allowlist para não virar proxy aberto de URL); quando presente, o link sempre vence lat/lng digitados manualmente (`applyLocationLink` em `partner-input.ts`), com o mesmo arredondamento de ~1km já exigido pelo CLAUDE.md §14. **(2) Consentimento LGPD como caixas de seleção** — os campos de texto livre "Documento" e "Observações" viraram duas caixas de seleção com frase fixa (`src/lib/content/partner-private-input.ts`): "Documento físico assinado, arquivado na empresa." e "Representante autoriza a divulgação de seus dados de contato, em conformidade com a LGPD." (frase de autorização redigida nesta sessão); os valores continuam nas mesmas colunas `document`/`consentNotes` já existentes, sem migration. **(3) Seletor de municípios por busca** — `CommercialAreaMunicipalitiesForm` trocou a grade de ~1.191 checkboxes (uma por município da Região Sul) por um combobox de busca-e-adicionar (digita o nome, resultado aparece em lista `role=listbox`, clique ou Enter adiciona um chip removível); elimina de raiz a classe de bug já corrigida antes (perda de seleção ao filtrar), porque a seleção agora é estado controlado por array de IDs, não mais `defaultChecked` em checkboxes que podem desmontar. `ui-reviewer` revisou os três pontos: achado aplicado nos dois checkboxes novos do LGPD (faltava `h-4 w-4 accent-brand-orange` e `min-h-11` no `<label>`, padrão usado em todo checkbox do painel); demais achados eram registros não-bloqueantes (peso de fonte do texto legal, ausência de `aria-describedby` já pré-existente no arquivo). `lint`, `typecheck` e `npm run test:unit` (108 arquivos/529 testes, incluindo o novo `PartnerForm.test.tsx` que não existia antes) aprovados. Mostrar o nome dos municípios atendidos por parceiro **no site público**, e o mapa interativo (Leaflet) de busca de representante, ficam para a próxima entrada — dependem da seleção explícita de campos públicos do Fase 5, ainda não decidida. |
| 12/09/2026 | `scope-gate-reviewer` revisou o plano do Ponto 4 (mapa público) antes de qualquer implementação e apontou, corretamente, que o portão documentado da Fase 5 (conclusão da Fase 4 + seleção explícita de campos públicos) ainda não tinha sido cruzado no Plano Mestre, mesmo com a aprovação verbal de Jose para "implementar tudo, pontos 1 a 4" — a especificação de UX também divergia da registrada em 30/08/2026 (painel deslizante vs. página dedicada). Apresentadas 4 perguntas a Jose, respondidas no mesmo dia: (1) encerrar a Fase 4 formalmente agora e abrir a Fase 5; (2) aprovar a lista completa de campos públicos de `Partner` — nome, tipo, WhatsApp, site, redes sociais, descrição, coordenada aproximada, logo, municípios atendidos, nunca dados de `PartnerPrivate`; (3) confirmar o painel deslizante sobre a Home (com efeito de transição suave) como especificação vigente, substituindo a de 30/08/2026; (4) implementar rate limiting básico por IP antes de lançar a rota pública (Fase 6 continua responsável por anti-scraping mais robusto). Fase 4 e 5 atualizadas na seção 0.2 e nos cabeçalhos das respectivas seções; decisão de UX registrada como nota na Parte II (linha da especificação de 30/08/2026). |
| 12/09/2026 | Implementado o Ponto 4 (mapa público) com as 4 decisões acima. Sem migration de schema para `Partner`/`CommercialArea`/geografia (dado já existente); nova migration `20260912100000_fase5_public_partners_map` só adiciona `PARTNERS_READ` a `RateLimitContext` (reaproveita a infraestrutura de rate limit já existente do login, `src/lib/auth/rate-limit.ts`). Novo `src/lib/content/partner-public-repository.ts` com seleção explícita de campos (`select`, nunca `include`) e `checkPublicPartnersRateLimit` (IP, 30 req/min); nova rota `GET /api/public/partners` — primeira rota pública dinâmica do projeto. Novas dependências `leaflet`/`react-leaflet`/`@types/leaflet` (já listadas como stack aprovada em `CLAUDE.md` §4). `PresenceSection` virou client component: o botão "Encontrar representante" e a imagem estática do mapa agora abrem `RepresentativeMapPanel` (novo, `src/components/representatives/`) em vez de abrir o diálogo do WhatsApp direto — painel modal com o mesmo padrão obrigatório de `WhatsAppDialog`/`ContentArticleDialog` (`DESIGN-SYSTEM.md` §13: ESC fecha, foco preso, foco vai ao botão de fechar, `body.dialog-open` trava o scroll), mapa Leaflet (`LeafletMap`, dynamic import `ssr:false`) com geolocalização do visitante (marcador "Minha localização", nunca enviada ao servidor) e marcadores de parceiros, busca por cidade/nome, card do parceiro selecionado com link `wa.me` próprio (`buildPartnerWhatsAppLink`, mesmo padrão de `buildProductWhatsAppLink`). **Dois achados de segurança/funcionalidade só detectados em verificação manual no navegador** (não pegos por lint/typecheck/testes, já que dependiam do CSP real do `next.config.ts`, não exercitado pelos testes de componente): (1) `Permissions-Policy` já tinha `geolocation=()` bloqueando a API de geolocalização para toda a origem — o recurso central do Ponto 4 simplesmente não funcionaria em produção; corrigido para `geolocation=(self)`; (2) `Content-Security-Policy`'s `img-src` não incluía os subdomínios de ladrilho do OpenStreetMap, então o mapa carregaria permanentemente em branco/cinza; corrigido adicionando `https://*.tile.openstreetmap.org` (`src/lib/security/headers.ts`, com 2 novos testes). `ui-reviewer` também encontrou e foram corrigidos: título do painel colidindo com o botão de fechar; imagem do mapa estático perdendo centralização ao virar `<button>` clicável; ausência de foco preso (Tab/Shift+Tab) e de trava de scroll do `body`, diferente dos dois modais já existentes no site — ambos implementados seguindo o mesmo padrão de `WhatsAppDialog`. `lint`, `typecheck`, `npm run test:unit` (112 arquivos/551 testes) e `npm run test:db` (57 testes) aprovados; verificação manual em navegador confirmou painel abrindo/fechando, ladrilhos do mapa carregando e ausência de erros de CSP/Permissions-Policy após a correção. Pendências abertas para o fechamento formal da Fase 5: rate limiting mais robusto/anti-scraping (Fase 6), e `docs/API.md`/`docs/CODEBASE_MAP.md` seguem desatualizados quanto à API administrativa e pública já existentes (débito de documentação anterior a esta sessão, não introduzido por ela). |
| 12/09/2026 | Jose aprovou commit e push desta fatia (pontos 1-4). Commit `bdc712d` na `main`, push concluído. Migration `20260912100000_fase5_public_partners_map` aplicada em produção via Supabase MCP (confirmada em `list_migrations`) — sem esse passo manual, `GET /api/public/partners` falharia em produção assim que o deploy da Vercel terminasse, pelo mesmo motivo já registrado de o build da Vercel não rodar `prisma migrate deploy`. |
