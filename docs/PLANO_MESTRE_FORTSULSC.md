...
| 🟡 | Fase 2 — Estrutura Next.js | Tarefas 1A e 1B concluídas e commitadas (`57a414f`, `160eb38`): fundação Next.js/TypeScript + estrutura mínima do App Router (`src/app`), com `agentRules: false` registrado como decisão do Jose para preservar `CLAUDE.md`. Ainda não há migração do design real da home — `page.tsx` é só um placeholder. | Tarefa 2 — disponibilizar assets em `public/` sem remover o baseline estático; depois, checkpoint de fundação revisado pelo Claude antes da Tarefa 3 (migração visual). As sete decisões pendentes do planejamento vigente continuam bloqueando os incrementos que dependam delas. |
