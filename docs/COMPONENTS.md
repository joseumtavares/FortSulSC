...
- Pendência operacional (não é bug de código): busca por **representante** para uma cidade só encontra alguém cujo(s) área(s) comercial(is) já têm aquele município vinculado via `CommercialAreaMunicipalitiesForm` no admin — verificado em produção que o único parceiro real ainda não tem nenhum município vinculado à sua área comercial. Precisa de cadastro no admin, não de correção de código.
- Autocomplete de cidade: a busca reconhece qualquer um dos 1.191 municípios da Região Sul (PR/SC/RS) enquanto o visitante digita, mostrando sugestões (`CitySearchBox`, `src/lib/content/regiao-sul-municipios.ts`) — independente de já haver representante vinculado àquela cidade ou não. Dado embutido no bundle a partir da mesma fonte oficial do IBGE já usada para semear o banco (`prisma/data/geografia-sul.json`), sem chamada à API do IBGE em tempo real nem consulta ao banco — pedido do Jose (12/09/2026, Plano Mestre) para a busca não depender de as áreas comerciais já estarem todas vinculadas. Selecionar uma sugestão preenche a busca com o nome oficial do município; se nenhum parceiro atender aquela cidade, aparece o fallback de contato da FortSul (ver acima) já com o nome correto da cidade.
